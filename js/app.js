...
function loadProfile() {
  try { profile = JSON.parse(localStorage.getItem(STORAGE_PROFILE)); }
  catch { profile = null; }
}
...
function saveSession() {
  ...
  if (editingSessionId !== null) {
    ...
    if (idx !== -1) {
      session.id = editingSessionId; // mantener el id original
      session.createdAt = sessions[idx].createdAt; // mantener fecha de creación
